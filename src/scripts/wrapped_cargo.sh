#!/bin/sh
# wrapped_cargo.sh - Wrapper for cargo with +toolchain support.
...
# Execute cargo with or without toolchain specification.
if [ -n "$toolchain" ]; then
  exec /petros/bin/rustup run "$toolchain" cargo "$@"
else
  exec /petros/bin/cargo "$@"
fi
